...
|---|---|---|---|---|
| `local` (default) | no | keyword (FTS5) | your key (reasoning only) | zero-dep demo / offline |
| `cognee` | yes (local SDK) | yes | **your** OpenAI key | self-hosted Cognee |
| `cognee_cloud` | yes (hosted) | yes | **managed by Cognee** | the complete superset |
...
|---|---|---|---|---|
| 1 | **remember / store** | yes | `checkpoint` stores verbatim; on cognee backend calls `cognee.add` | `continuum checkpoint chat.txt -p X`, "save this chat" |
| 2 | **recall / retrieve** | yes | `search` + resume retrieval; cognee backend = semantic, local = keyword | `continuum search -p X "redis"`, "search project X for redis" |
...
| 4 | **knowledge graph extraction** | yes built / ~ used | `cognee.cognify()` builds the KG; resume doesn't yet *query* it | automatic on checkpoint (cognee backend) |
| 5 | **vector / semantic retrieval** | yes (cognee) | `SearchType.CHUNKS` via embeddings; local backend = FTS5 keyword | automatic in `search`/`resume` on cognee backend |
| 6 | **improve / refine** | yes | `improve` = reasoning-graph memify (dedup, resolve superseded) | `continuum improve -p X`, "improve project X" |
| 7 | **forget / delete** | yes | `forget` = local cascade + `cognee.prune.prune_data` | `continuum forget -p X`, "forget project X" |
| 8 | **dataset / project scoping** | yes superset | project scoping **+ per-user multi-tenancy** (`{user}::{project}`) | `continuum --user alice ... -p X`, `X-Continuum-User` header |
| 9 | **CLI + UI surfaces** | yes / ~ | full CLI; product **dashboard** (`site/`); inspection via `status`/`timeline`/`search` (no graph-explorer UI) | `continuum status/timeline -p X` |
| 10 | **SDK / API integration** | yes | Python `Continuum` engine + FastAPI HTTP API | `from continuum import Continuum`, `continuum serve` |
| 11 | **MCP / plugin integration** | yes superset | **18 MCP tools**, stdio + remote | see [INTEGRATIONS.md](INTEGRATIONS.md) |
| 12 | **backend / config flexibility** | yes | env-driven: `local` to `cognee`, LLM keys, budgets, tenancy | see `.env.example` / [GETTING-STARTED.md](GETTING-STARTED.md) |
