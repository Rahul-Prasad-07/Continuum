...
| Env | Default | Purpose |
|---|---|---|
| `OPENAI_API_KEY` / `ANTHROPIC_API_KEY` | — | clean LLM extraction |
...
| `CONTINUUM_TOKEN` | — | require bearer auth on API + remote MCP |
...
**MCP** (any AI), and **HTTP**. See [USE-CASES.md](USE-CASES.md) for a recipe per verb.
